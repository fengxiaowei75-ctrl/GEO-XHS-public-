...
GATEWAY_SERVICE_TOKEN=<central-api-cost-gateway service token>
```

不要把 `PGPASSWORD` 或 `GATEWAY_SERVICE_TOKEN` 提交到代码仓库。
Vercel 必须配置中央网关的公网 HTTPS 地址；生产环境缺少网关变量时，消耗型接口会失败关闭。
